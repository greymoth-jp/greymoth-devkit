...
- [ ] T1 実態確認：4 explorer の実コード/値を統合（task#1）
- [ ] T2 scaffold：root package.json(workspaces)/tsconfig.base/.gitignore/LICENSE/git init
- [ ] T3 design direction：design-ref で identity 確定（task#3）
- [ ] T4 `@greymoth/tokens`：実 token を単一 source 化 + test
- [ ] T5 `@greymoth/rules-core`：Rule/Finding/evaluator/rollup + test
- [ ] T6 `@greymoth/scorecard`：score+verdict-stamp render + test
- [ ] T7 `@greymoth/sharecard`：1200×630 SVG カード + test
- [ ] T8 demo：実合成 pipeline（rules→score→verdict→card）
- [ ] T9 story landing：frontend-design + headless 実描画検証
- [ ] T10 AUDIT 100%：build+test 全 green
- [ ] T11 README（物語）+ docs/LAUNCH.md（配布草稿）+ REFERENCES.md
- [ ] T12 git commit + tag 復元点 / memory 刻む
